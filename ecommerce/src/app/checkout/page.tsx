...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (sess) {
      ...
      setCustomer(s)
      setShippingAddress(s.companyName)
    }
    if (ids.length > 0) {
  ...
  const handleSubmit = async () => {
    if (!customer) { router.push("/auth/login?redirect=/checkout"); return }
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.error || "Order gagal")
  ...
  if (success) return (
    <div className="max-w-lg mx-auto px-4 py-20 text-center">
      ...
      <p className="text-gray-500 mt-2">Pesanan <strong className="text-gray-900">{orderNumber}</strong> telah dikirim.</p>
      <p className="text-sm text-gray-400 mt-1">Kami akan memproses pesanan Anda dan mengirimkan invoice.</p>
      <div className="flex items-center justify-center gap-3 mt-6">
        <Link href="/orders"><Button>Lihat Pesanan</Button></Link>
        <Link href="/"><Button variant="outline">Belanja Lagi</Button></Link>
      </div>
    </div>
  )
        ...
        <div className="md:col-span-3 space-y-6">
          {customer ? (
            <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-2 text-sm text-green-700">
              <CheckCircle2 className="h-4 w-4" /> Login sebagai <strong>{customer.companyName}</strong>
            </div>
          ) : (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center justify-between">
              <span className="text-sm text-amber-700"><AlertCircle className="h-4 w-4 inline mr-1" /> Silakan login untuk checkout</span>
              <Link href="/auth/login?redirect=/checkout"><Button size="sm" variant="outline">Login</Button></Link>
            </div>
          )}

          <div className="space-y-3">
            <h3 className="font-medium text-gray-900">Item Pesanan</h3>
            {cartItems.map((item) => (
              <div key={item.id} className="flex items-center justify-between bg-white border border-gray-200 rounded-lg p-3">
                <div className="flex items-center gap-3">
                  <Package className="h-8 w-8 text-gray-300" />
                  <div><p className="text-sm font-medium">{item.name}</p><p className="text-xs text-gray-400">{item.sku} × {item.quantity}</p></div>
                </div>
        ...
        <div className="md:col-span-2">
          <div className="bg-gray-50 rounded-xl p-6 space-y-4 sticky top-24">
            ...
            <Button className="w-full" onClick={handleSubmit} disabled={submitting || !customer}>
              {submitting ? <><Loader2 className="h-4 w-4 animate-spin mr-2" /> Memproses...</> : <><ShoppingCart className="h-4 w-4 mr-2" /> Buat Pesanan</>}
            </Button>
          </div>
        </div>
      </div>
