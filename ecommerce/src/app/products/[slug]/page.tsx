    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <p className="text-3xl font-bold text-gray-900">{formatCurrency(product.price)} <span className="text-sm font-normal text-gray-400">/ {product.uom}</span></p>
          ...
          <div>
            <h3 className="text-sm font-medium text-gray-700 mb-2">Stok Tersedia</h3>
            {product.warehouseStock?.map((ws: any) => (
              <div key={ws.warehouse} className="flex justify-between text-sm py-1">
            ...
            <div className="flex items-center gap-2 mt-2 text-sm">
              ...
              <span className={product.totalStock > 0 ? "text-green-600" : "text-red-600"}>
                {product.totalStock > 0 ? `${product.totalStock} unit tersedia` : "Stok habis"}
              </span>
            </div>
          </div>
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center border border-gray-300 rounded-lg">
                  ...
                  <Input type="number" min={1} max={product.totalStock} value={quantity}
                    onChange={(e) => setQuantity(Math.max(1, parseInt(e.target.value) || 1))} className="w-16 text-center border-0 h-10" />
                ...
                <Button onClick={addToCart} className="flex-1 flex items-center gap-2" disabled={added}>
                  {added ? <><CheckCircle2 className="h-4 w-4" /> Ditambahkan!</> : <><ShoppingCart className="h-4 w-4" /> + Keranjang</>}
                </Button>
              </div>
          ...
          <div className="border-t border-gray-200 pt-4">
            <p className="text-xs text-gray-400">Harga B2B sudah termasuk grosir. Faktur pajak tersedia untuk pelanggan terdaftar.</p>
          </div>
        </div>
      </div>
    </div>
